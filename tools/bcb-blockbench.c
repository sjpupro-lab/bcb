...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i + 1 < argc; i++) {
        if (!strcmp(argv[i], "--prior")) prior_path = argv[++i];
        else if (!strcmp(argv[i], "--blocks")) blocks_path = argv[++i];
    }
    if (!prior_path || !blocks_path) {
        fprintf(stderr, "usage: %s --prior <file> --blocks <file>\n", argv[0]);
        return 2;
    }
    ...
    while (read_u32(bf, &len)) {
        ...
        printf("%u %zu\n", len, clen);
        total_orig += len; total_comp += clen; nblocks++;
        free(dec); free(comp); free(msg);
    }
    fclose(bf);

    fprintf(stderr, "blocks=%ld lossless=%s total_orig=%zu total_comp=%zu ratio=%.3f enc_ms_per_block=%.3f blocks_per_s=%.0f\n",
            nblocks, lossless ? "yes" : "NO", total_orig, total_comp,
            total_comp ? (double)total_orig / total_comp : 0.0,
            nblocks ? enc_sec * 1000.0 / nblocks : 0.0,
            enc_sec > 0 ? nblocks / enc_sec : 0.0);
    bcb_prior_close(p);
    return lossless ? 0 : 3;
}
