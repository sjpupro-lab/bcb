...
struct BcbEncoder { BcbCodec *c; int crc_on; };
...
static ssize_t compress_core(BcbCodec *c, int crc_on, const uint8_t *in, size_t in_len,
    ...
    uint8_t hdr[16]; int hn = 0;
    hdr[hn++] = (uint8_t)(crc_on ? 1 : 0);
...
static ssize_t decompress_core(BcbCodec *c, const uint8_t *in, size_t in_len,
    ...
    off += (size_t)hn;
    uint32_t want = 0; int has_crc = tag & 1;
    if (has_crc) {
...
ssize_t bcb_compress(BcbPrior *p, const uint8_t *in, size_t in_len, uint8_t *out, size_t cap) {
    ...
    if (!c) return BCB_ERR_INVALID_PRIOR;
    ssize_t r = compress_core(c, 1, in, in_len, out, cap);
    bcb_codec_free(c);
    return r;
}
...
ssize_t bcb_encode(BcbEncoder *e, const uint8_t *in, size_t in_len, uint8_t *out, size_t cap) {
    if (!e) return BCB_ERR_INVALID_PRIOR;
    return compress_core(e->c, e->crc_on, in, in_len, out, cap);
}
