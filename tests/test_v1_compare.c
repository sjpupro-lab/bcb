...
static size_t run(CecBT bt, CecBT bt2,
...
int main(int argc, char **argv) {
    ...
    for (size_t t = 0; t < sizeof(trains)/sizeof(trains[0]); t++) {
        ...
        int ok0 = 0, ok1 = 0;
        size_t c0 = run(cec_default_bt(), cec_default_bt(), corpus, toff, tl, ex, ex_len, &ok0);
        size_t c1 = run(symdist_bt(),     symdist_bt(),     corpus, toff, tl, ex, ex_len, &ok1);
        all_ok = all_ok && ok0 && ok1;

        char lbl[16]; snprintf(lbl, sizeof(lbl), "%zuKB", tl/1000);
